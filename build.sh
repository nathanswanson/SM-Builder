...
docker buildx build --ssh default --platform linux/amd64 -t server-manager:amd64 --load .
# save the image to a tar file
docker save server-manager:amd64 -o server-manager-amd64.tar

# optional deploy to server
if [ "$1" == "deploy" ]; then
  scp server-manager-amd64.tar game-server@raspberrypi.home:/home/game-server/deploy/
  ssh game-server@raspberrypi.home "docker load -i /home/game-server/deploy/server-manager-amd64.tar"
